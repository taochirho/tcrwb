...
import com.taochirho.testvm.application.TAG_SORTED_COUNT
import com.taochirho.testvm.application.TAG_SHUFFLED_COUNT
...
//        Log.w("Tile.toString", "$letter $state $tilePos")
...
class GameRepository(private val gameDao: GameDao, private val currentGameDao: CurrentGameDao) {
  ...
  //  suspend fun clearAndResetCount() = gameDao.clearAndResetCount()
    ...
    suspend fun deleteCurrent() = currentGameDao.deleteCurrent()
...
class GameRepository internal constructor(gameInitDao: GameDao)  {
    //
//    val allGames: LiveData<List<Game>> = gameInitDao.getAllGames()
//
    private val gameDao: GameDao = gameInitDao
...
//    suspend fun getGameTilesWithUid(key: Int) = gameDao.getGameTilesWithUid(key)
...
//    suspend fun update(game: Game) = gameDao.update(game)
    ...
    suspend fun clear() = gameDao.clearAndResetCount()
